...
with st.expander("⚙️ Configurações da Proposta (nome, logo, contato)", expanded=not perfil["nome_empresa"]):
    ...
    with col_p2:
        ...
        logo_upload = st.file_uploader("Logo (opcional)", type=["png", "jpg", "jpeg"], key="upload_logo")
    ...
    if st.button("💾 Salvar Configurações", key="btn_salvar_perfil"):
...
with st.expander("💲 Personalizar Tabela de Preços (avançado)"):
    ...
    if overrides_atuais:
        ...
        if st.button("↩️ Restaurar preços padrão (desfazer todas as customizações)", key="btn_restaurar_precos"):
...
with col_pad:
    padrao = st.selectbox("Padrão de Acabamento", ["Econômico", "Médio", "Alto Padrão"], key="select_padrao")
with col_est:
    estrutura = st.selectbox("Tipo de Cobertura", ["Telhado", "Laje"], key="select_estrutura")
...
if arquivo_pdf is not None:
    ...
    if "dados_extraidos" in st.session_state:
        ...
        if st.button("🚀 Gerar Orçamento Completo", key="btn_gerar_orcamento"):
